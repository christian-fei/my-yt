...
function runFfmpegInWorker (location, format, duration, callback) {
  return new Promise((resolve, reject) => {
    ...
    worker.on('message', (message) => {
      ...
      if (message.status === 'progress') {
        console.log('HERE WE ARE message', message)
        callback(message)
      }
    })
...
export async function downloadVideo (id, repo, callback = () => {}) {
  return new Promise(async (resolve, reject) => {
    ...
    try {
      ...
      await runVideoInWorker(id, quality, (message) => {
        if (message.status === 'info') {
      ...
      if (repo.getTranscodeVideosSetting() && video.location && video.format) {
        await runFfmpegInWorker(video.location, video.format, parseDuration(video.duration), (message) => {
          console.log(message.line)
          callback(message)
        })
...
export async function getVideosFor (channelName) {
  ...
  function toInternalVideo (v) {
    ...
    if (!data) {
      console.warn('Missing videoRenderer data:', v)
      return
    }

    // Ensure viewCountText exists before accessing simpleText
    const viewCount = data.viewCountText?.simpleText || '0 views'

    return {
      channelName,
      title: data.title?.runs[0]?.text || 'Untitled',
      url: `https://www.youtube.com/watch?v=${data.videoId}`,
      thumbnail: `https://img.youtube.com/vi/${data.videoId}/mq2.jpg`,
      description: data.descriptionSnippet?.runs[0]?.text || 'No description',
      id: data.videoId,
      publishedTime: data.publishedTimeText?.simpleText || 'Unknown time',
      publishedAt: parseRelativeTime(data.publishedTimeText?.simpleText),
      viewCount,
      duration: data.lengthText?.simpleText || 'Unknown duration'
    }
  }
}
