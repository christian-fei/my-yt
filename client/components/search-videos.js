...
class SearchVideos extends HTMLElement {
  ...
  searchHandler (event) {
    ...
    const query = `?filter=${encodeURIComponent(searchTerm)}${excluded ? '&excluded=true' : ''}${downloaded ? '&downloaded=true' : ''}${ignored ? '&ignored=true' : ''}${summarized ? '&summarized=true' : ''}${bookmarked ? '&bookmarked=true' : ''}`
